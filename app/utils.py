...
def load_checkpoint(checkpoint, model, optimizer=None):
    if isinstance(checkpoint, str):
        assert os.path.isfile(checkpoint), "No checkpoint found at '{}'".format(
            checkpoint)
...
def get_hparams_from_file(config_object):
    ...
    if isinstance(config_object, str):
        ...
        data = config_object.read()
        config = json.loads(data)
    else:
        raise TypeError(
            f"config_object must be str or bytes, but {type(config_object)} is given.")
    hparams = HParams(**config)
    return hparams
...
def find_path_by_suffix(dir_path: pathlib.Path, suffix: pathlib.Path):
